class Solution {
    ...
    //visitedTime -> keeps track of when a node is discovered
    ...
    void DFSHelper(int node) {
        ...
        for(int i = 0; i < adjList[node].size(); ++i)
        {
            ...
            //check just for a single back-edge
                ...
                low[node] = min(low[node],visitedTime[child]);
        }
    }
public:
